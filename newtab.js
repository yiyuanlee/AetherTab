...
function initWeather() {
  ...
  if (isChromeExtension) {
    chrome.storage.local.get(['weatherLocation', 'weatherUnit'], (result) => {
      weatherLocation = result.weatherLocation || null;
      weatherUnit = result.weatherUnit || 'C';
      startWeatherFlow();
    });
  } else {
    weatherLocation = JSON.parse(localStorage.getItem('weatherLocation')) || null;
    weatherUnit = localStorage.getItem('weatherUnit') || 'C';
    startWeatherFlow();
  }
  ...
  // Switch to Manual Search View
  cityBtn.addEventListener('click', () => {
    showView('search');
  });

  // Click on location label to edit/search city manually
      ...
      fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(query)}&count=1&language=en&format=json`)
        ...
        .then(data => {
          if (data.results && data.results.length > 0) {
            ...
            saveLocationState(result.latitude, result.longitude, name);
  ...
  tempLabel.addEventListener('click', () => {
    ...
    if (isChromeExtension) {
      chrome.storage.local.set({ weatherUnit });
    } else {
  ...
  function saveLocationState(lat, lon, name) {
    weatherLocation = { lat, lon, name, timestamp: Date.now() };
    if (isChromeExtension) {
      chrome.storage.local.set({ weatherLocation });
    } else {
  ...
  function renderWeather(data, locationName) {
    ...
    locationLabel.title = `Click to change location (Current GPS: ${data.latitude.toFixed(2)}, ${data.longitude.toFixed(2)})`;
    ...
    iconContainer.innerHTML = weatherInfo.iconSvg;
